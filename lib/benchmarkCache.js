import { BENCHMARK_WORKERS, fetchBenchmarkData } from "./rpcBenchmark";
  ...
  for (const sample of samples) {
    for (const rpc of sample) {
      ...
      for (const response of rpc.responses) {
        ...
        methodStats.lastRequest = response.request ?? methodStats.lastRequest;
    ...
    responses: Array.from(endpoint.methods.values()).map((methodStats) => {
      ...
      if (methodStats.successCount === 0) {
        return {
          ...
          request: methodStats.lastRequest,
          result: null,
          error: true,
          errorMessage:
...
const runBenchmark = async () => {
  ...
  try {
    ...
    cache = {
      ...
      firstSampleAt: cache.firstSampleAt ?? fetchedAt,
      fetchedAt,
      error: null,
    };
  } catch (error) {
...
export const getBenchmarkSnapshot = async () => {
  ...
  return {
    data: cache.data,
    firstSampleAt: cache.firstSampleAt,
    fetchedAt: cache.fetchedAt,
    error: cache.error,
    running: cache.running,
    intervalMs: BENCHMARK_INTERVAL_MS,
    workerCount: BENCHMARK_WORKERS,
    sampleCount,
